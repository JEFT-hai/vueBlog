<template>
  <div class="postActivityPage">
    ...
    <!-- 投票规则 -->
    <jeft-editor :show="hasMask && rulesShow"
                 @hideEditor="hideRulesMask"
                 postName="保存"
                 :detail="activity.rulesContent"></jeft-editor>
    <!-- 海报 -->
    ...
    <div class="voteClassification">
      <div class="voteSign">
        <p>是否需要IMEI(手机唯一标识号)</p>
        <div class='voteOne voteBox'
             @click="handleSign(true, 1)">
          <span>
            需要
          </span>
          <span :class="['chooseRight',{isRight: isNeedSign}]">
            <i class="iconWanwan iconWzhengquecopy activityNotice"></i>
          </span>
        </div>
        <div class='voteTwo voteBox'
             @click="handleSign(false, 2)">
          <span>
            不需要
          </span>
          <span :class="['chooseRight',{isRight: !isNeedSign}]">
      ...
      <div class="voteSign">
        ...
        <div class='voteOne voteBox'
          ...
          <span :class="['chooseRight',{isRight: voteType === 0}]">
            <i class="iconWanwan iconWzhengquecopy activityNotice"></i>
          </span>
        </div>
        <div class='voteTwo voteBox'
          ...
          <span :class="['chooseRight',{isRight: voteType === 1}]">
            <i class="iconWanwan iconWzhengquecopy activityNotice"></i>
          </span>
        </div>
      </div>
    ...
    <div class="">
      <ul class="activityBaseInfo LRForm">
        ...
        <li class="LRFormItem borderB marginTL"
            @click="setVoteRules(true)">
          <span class="LFormItem">投票规则</span>
        ...
        <!-- <li class="LRFormItem borderB">
            <span class="LFormItem">活动详情1</span>
            <div class="inputWrapper FormItemContent canClear">
              <input type="text" maxLength='30' v-model="detail[0]" placeholder="不超过30个字" />
            </div>
          </li>
          <li class="LRFormItem borderB">
            <span class="LFormItem">活动详情2</span>
            <div class="inputWrapper FormItemContent canClear">
              <input type="text" maxLength='30' v-model="detail[1]" placeholder="不超过30个字" />
            </div>
          </li>
          <li class="LRFormItem borderB">
            <span class="LFormItem">活动详情3</span>
            <div class="inputWrapper FormItemContent canClear">
              <input type="text" maxLength='30' v-model="detail[2]" placeholder="不超过30个字" />
            </div>
          </li>
          <li class="LRFormItem borderB">
            <span class="LFormItem">活动详情4</span>
            <div class="inputWrapper FormItemContent canClear">
              <input type="text" v-model="detail[3]" placeholder="无字数限制" />
            </div>
          </li> -->
        <li @click="postActive"
...
export default {
  ...
  computed: {
    ...
    rulesContent () {
      if (!this.activity.rulesContent) {
        return
      }
      return this.activity.rulesContent.replace(/&lt;/g, '<').replace(/&gt;/g, '>')
    }
  },
  created () {
  ...
  methods: {
    ...
    handleSign (value,index) {
      this.isNeedSign = value
      this.needIndex = index
      if (index === 1) { // 表示用户选择了需要认定，那么投票的方式就是一个一共只有一票
        console.log('表示需要认定')
        this.voteType = 1
      }
    },
    ...
    modelCallBackDraft (data) {
      if (data) {
        ...
        p.then(() => {
          ...
          this.activity.saveTime = this.format.date(new Date(), 'yyyy-MM-dd hh:mm')
          // 进入页面启用草稿箱
    ...
    postActive () {
      ...
      p.then(() => {
        let data = {
          ...
          isImei: me.isNeedSign, // 标识投是否需要IMEI,手机唯一标识号
          voteType: me.voteType, // 投票的类型 0标识每天每人1票，1标识每人总共一票
    ...
    setVoteRules (data) { // 设置投票规则编辑器的显示
    console.log('我是规则呢')
      this.setMask(data)
      this.rulesShow = data
    },
    hideEditorMask (data) { // 设置投票详情编辑器的隐藏
      data && this.$set(this.activity, 'detailContent', data)
      // this.activity.detailContent = data
      this.setEditorMask(false)
    },
    hideRulesMask (data) { // 设置投票规则编辑器的隐藏
      console.log('我是关闭规则')
      data && this.$set(this.activity, 'rulesContent', data)
      // this.activity.detailContent = data
      this.setVoteRules(false)
    },
    backActivityHome () {
    ...
    .voteSign{
      ...
      .voteBox{
        ...
        .isRight{
          ...
          border:1px solid rgb(255, 72, 95);
        }
      }
    }
  }
