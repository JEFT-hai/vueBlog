  ...
  <div class="postActivityPage">
    ...
    <div class>
      <ul class="activityBaseInfo LRForm">
        ...
        <li class="LRFormItem borderB"
            @click="setChooseGroup(true)">
          <span class="LFormItem">主办群</span>
          <p class="FormItemContent"
             v-if="activity.group">{{activity.group.groupName}}</p>
          <p class="FormItemContent"
             v-else></p>
          <i class="iconWperson-fill iconWanwan"></i>
        </li>
        <li class="LRFormItem borderB"
            @click="setAddressMask(true)">
          <span class="LFormItem">活动地点</span>
          <p class="FormItemContent"
             v-if="activity.address">{{activity.address.province}}{{activity.address.city}}{{activity.address.district}}</p>
          <p class="FormItemContent"
             v-else></p>
          <p class="RText">
        ...
        <li class="LRFormItem borderB">
          ...
          <input class="FormItemContent"
                 v-if="activity.address"
                 v-model="calculateCity" />
          <p class="FormItemContent"
             v-else></p>
        </li>
        <li class="LRFormItem borderB"
          ...
          <p class="FormItemContent">
            <span v-if="activity.typeTag">{{activity.typeTag[0]}}</span>
          </p>
          <p class="RText">
            <span v-show="!activity.typeTag || (activity.typeTag && !activity.typeTag.length)">请选择</span>
            <i class="iconWxiayibu iconWanwan textR"></i>
          </p>
        </li>
        <li class="LRFormItem borderB">
        ...
        <li @click="postActive"
          ...
          <div :class="canPost?'submitPostBtn canSubmitPost':'submitPostBtn'">发布活动</div>
        </li>
      </ul>
    </div>
    <div class="saveActive"
         @click.self="saveActive">存草稿</div>
  </div>
</template>
<script>
// FIXME:时间选择可以优化
...
export default {
  ...
  computed: {
    ...
    hasContent () {
      return (this.activity.filesResults || this.activity.title || this.activity.address || this.activity.detailContent)
    },
    detailContent() {
  ...
  methods: {
     ...
     */
    testHasDrafts () {
      if (this.draftsIndex >= 0) { // 有启动草稿(index)就启用草稿并删除该草稿
        this.activity = JSON.parse(localStorage.getItem('activeDrafts'))[this.draftsIndex]
        ...
        return
      } else if (!this.shouldShowModel) { // 从草稿箱回来不显示启动草稿箱model
        return
      } else if ( // 显示启动草稿箱model
      ...
      return new Promise((resolve, reject) => {
        if (
          ...
          me.api.file({
            ...
            data: fd,
            success: res => {
              me.$set(me.activity, 'filesResults', res)
              resolve()
            },
            error: res => {
              reject(res)
    ...
    setAddress(data) {
      ...
      let a1 = this.activity.address && this.activity.address.curTit
      let a2 = this.activity.address && this.activity.address.curAddress
      this.calculateCity = a1 + a2
    ...
    selectPicker(pickerSelectedVal, selectedIndex) { // 选择时间
      this.curPicker === 0 &&
    ...
    postActive() {
      ...
      if (!this.activity.group || !this.activity.group.userGroupId) {
        this.$toast('请输入主办群')
        return
      }
      ...
      if (!this.calculateCity) {
        this.$toast('请输入活动详细地点')
        return
      }
      if (
        !this.activity.typeTag ||
        (this.activity.typeTag && !this.activity.typeTag.length)
      ) {
        this.$toast('请输入活动类型')
        return
      }
      // if (!this.activity.tags || (this.activity.tags && !this.activity.tags.length)) {
      //   this.$toast('请输入活动标签')
      //   return
      // }
      ...
      me.isUploading = true
      let p = ''
      // let p = this.upFileThumbnailZijiRen()  // 压缩比例小
      if (me.$store.state.isZijiren) {  // 判断是不是PC, 
        p = this.upFileThumbnailZijiRen()
      } else {
        p = this.upFileThumbnail()
      }

      let activityTypeTagId = me.activeConditions.activetypes.filter(
        (item) => {
          return item.label === me.activity.typeTag[0]
        }
      )
      me.isFree && me.$set(me.activity, 'cost', 0)
      me.endCanJoin && me.$set(me.activity, 'joinEndTime', me.activity.endTime)
      let startTime = new Date(
        me.activity.startTime.replace(/\s*[\u4e00-\u9fa5]/g, '') + 'Z'
      )
      ...
      p.then(() => {
        me.$uploading(true, '上传中...')
        me.api.process({
          url: 'Activity',
          data: {
            title: me.activity.title,
            groupId: me.activity.group.userGroupId,
            activityCategoryId: activityTypeTagId[0].value,
            activityImg: me.activity.filesResults,
            beginTime: startTime,
            endTime: endTime,
            address: `${me.calculateCity}`,
