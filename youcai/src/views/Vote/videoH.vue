      ...
      <div class="videoMain">
        ...
        <div class="activityDesc">
          <div class="activityName borderB">
            <span>正在参加</span>
            <span @click='goVote'
                  class="activityTit">【{{item.title}}】</span>
          </div>
        </div>
        <div class="voteNums">
          <div>
        ...
        <div class="btnList borderB">
          <div @click="toVote"
               :class="['voteBtn', isVoted || isEnd ? 'isVoted' : '',  {'toggle__button--isLoading':isLoading}]">{{voteState}}</div>
          <div @click="share"
...
import ItemContent from '@/views/Discover/components/ItemOnlyFull.vue'
...
export default {
  ...
  computed: {
    ...
    isImei () {
      return this.postDetail && this.postDetail.isImei
    },
    isEnd () {
      return this.postDetail && this.postDetail.isEnd
    },
    voteState () {
      return this.isEnd ? '已结束' : this.isVoted ? '已投票' : '投票'
    },
    isInVoteList () {
  ...
  methods: {
    ...
    goVote () {
      if ((this.fName === 'voteList' || this.fName === 'voteState') && this.fId === this.postDetail.activityId) {
        this.$router.back()
      } else {
        this.$router.replace({
          name: 'voteList',
          params: {
            id: this.postDetail.activityId
          }
        })
      }
    },
    ...
    share () {
      ...
      let isDelete = this.item.userId === this.$store.state.user.userId
      let innerObj = {
        name: this.$route.name,
        objectId,
      }
      this.$share.open(this.item, isDelete, 'voteDetail', this.$router.currentRoute.fullPath, innerObj)
    },
    setVotedList (data) {
