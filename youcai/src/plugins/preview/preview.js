...
var vuePhotoPreview = {
  install(Vue, opts) {
    ...
    Vue.mixin({
      ...
      watch: { // 监听物理返回键
        '$store.state.hasMask': function (newV, oldV) {
          if (!newV && oldV && Vue.prototype.$preview.self) {
      ...
      methods: {
        ...
        onThumbnailsClick(e) {
          ...
          var clickedGallery = thumbElements // img
        ...
        async openPhotoSwipe(index, galleryElement, disableAnimation, fromURL) {
          ...
          items = await this.parseThumbnailElements(galleryElement) // 获取图片数据
          options = {
          ...
          gallery.listen('close', function () {
            $vue.$store.commit('setMask', false)
        ...
        initPreview(gallerySelector) {
          ...
          if (this.galleryElements.length && this.galleryElements[0].getAttribute('preview') === '0') { // 后加的过滤
            return
          }
          for (var i = 0, l = this.galleryElements.length; i < l; i++) {
            this.galleryElements[i].setAttribute('data-pswp-uid', i + 1)
